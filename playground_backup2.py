...
G_MPS2 = 9.80665  # standard gravity
...
class DroneSpecs:
    ...
    climb_efficiency: float = 0.6          # ASSUMED: fraction of ideal m*g*climb_rate that must
                                            # actually be spent as extra propulsion power when
                                            # climbing (rotor/motor losses); typical multirotor
                                            # climb figure of merit. Descending is treated as
                                            # zero extra cost, not negative -- multirotors can't
                                            # regenerate, and a slow controlled descent costs
                                            # roughly what level flight does, not less.
...
def climb_power_w(delta_h_m: float, time_s: float, d: DroneSpecs) -> float:
    """Extra power (W) needed to change altitude by delta_h_m over time_s.

    The lattice flies at a fixed height-AGL (CRUISE_AGL_M), not a fixed MSL
    altitude -- so wherever the terrain underneath an edge rises or falls,
    the drone has to climb or descend to keep station above it. That's real
    energy that the flat-terrain power model above has no way to see.
    Positive delta_h_m (climbing) costs m*g*climb_rate / climb_efficiency.
    Negative delta_h_m (descending) costs nothing extra here -- see the
    climb_efficiency field comment for why that's not modeled as a credit.
    """
    if time_s <= 0 or delta_h_m <= 0:
        return 0.0
    climb_rate_mps = delta_h_m / time_s
    return d.mass_kg * G_MPS2 * climb_rate_mps / d.climb_efficiency
...
# (offsets_km, derived per-route in lattice_sizing -- see section 5) --
# otherwise two genuinely different candidate lanes round into the same
# bucket and the optimizer loses the exact wind difference it's supposed to
# be finding. lattice_sizing() enforces MIN_LANE_SPACING_KM = 1.5 km as its
# floor specifically so it never gets finer than this cache bucket.
# 0.01 deg =~ 1.1 km, comfortably finer than that 1.5 km floor while still
...
_CACHE_LOCK = threading.Lock()  # wind and elevation are now fetched on parallel
                                 # threads (see main()); both write into the
                                 # same _CACHE dict/file, so serialize saves.
...
# How finely to sample the corridor scales with how long the corridor is --
# a fixed layer/lane count regardless of distance either wastes samples on a
# short hop or badly undersamples a long one (see lattice_sizing docstring).
TARGET_LAYER_SPACING_KM = 10.0  # ASSUMED: near HRRR's ~3km and in the same
                                # ballpark as ICON/GFS's ~2-25km native
                                # resolution -- sampling much finer than this
                                # mostly just re-reads the same interpolated
                                # wind, it doesn't add real information.
MIN_LAYERS = 3                 # ASSUMED: always keep at least one interior
                                # decision layer, even for a very short hop
MAX_LAYERS = 15                # ASSUMED: caps lattice/Dijkstra size and the
                                # number of distinct wind buckets fetched
N_LANES = 5                    # lateral offsets per interior layer (odd, includes center)

# CORRIDOR WIDTH -- how far sideways the lattice is allowed to search for
# wind. This is now sized from the route's total distance ONLY, not from
# how many along-track layers happen to subdivide it.
#
# FIX (was the cause of "adding more waypoints made results worse"): this
# used to be `layer_spacing_km * LANE_SPACING_FRACTION`, i.e. lane spacing
# was derived from layer spacing. That silently coupled two unrelated
# knobs: "how finely do I sample along the route" and "how far sideways can
# I search." Adding more layers shrinks layer_spacing_km (same total
# distance, more subdivisions) -- which shrank lane spacing right along
# with it, down to a MIN_LANE_SPACING_KM floor of just ±3 km total
# regardless of route length. Fewer layers meant fatter layer_spacing_km,
# which meant a *wider* corridor and more room to detour into a good wind
# lane -- so a coarser lattice wasn't finding a better answer over a
# bigger search space, it was accidentally searching a wider one while the
# "denser" lattice searched a narrower one. Decoupling them means more
# waypoints only ever adds resolution, never removes search space.
CORRIDOR_WIDTH_FRACTION = 0.15   # ASSUMED: corridor half-width as a fraction
                                  # of total A-to-B distance
MIN_CORRIDOR_HALF_WIDTH_KM = 3.0 # ASSUMED: floor so short hops still get a
                                  # meaningful detour option
MAX_CORRIDOR_HALF_WIDTH_KM = 20.0 # ASSUMED: cap how far sideways any route
                                  # is allowed to detour chasing wind
MIN_LANE_SPACING_KM = 1.5        # ASSUMED: stay comfortably above
                                  # CACHE_SPATIAL_DEG's ~1.1km bucket so
                                  # adjacent lanes don't collapse into the
                                  # same cached wind reading


def lattice_sizing(dist_km):
    """Pick (n_layers, offsets_km) so lattice resolution scales with route
    length, instead of a fixed 5-layer/±6km lattice regardless of whether
    A-B is 15 km or 1500 km apart. Along-track resolution (n_layers) and
    lateral search width (offsets_km) are sized independently -- see the
    CORRIDOR WIDTH comment above for why that independence matters."""
    n_layers = round(dist_km / TARGET_LAYER_SPACING_KM) + 1
    n_layers = max(MIN_LAYERS, min(MAX_LAYERS, n_layers))

    corridor_half_width_km = max(
        MIN_CORRIDOR_HALF_WIDTH_KM, min(MAX_CORRIDOR_HALF_WIDTH_KM, dist_km * CORRIDOR_WIDTH_FRACTION)
    )
    half = N_LANES // 2
    lane_spacing_km = max(MIN_LANE_SPACING_KM, corridor_half_width_km / half)
    offsets_km = tuple((k - half) * lane_spacing_km for k in range(N_LANES))
    return n_layers, offsets_km


def build_lattice(a, b, n_layers=None, offsets_km=None):
    """n_layers points along the direct track; each interior layer gets lateral offsets.
    If n_layers/offsets_km are omitted, both are derived from the A-B distance
    via lattice_sizing() rather than defaulting to a fixed size."""
    dist = haversine_km(*a, *b)
    if n_layers is None or offsets_km is None:
        auto_layers, auto_offsets = lattice_sizing(dist)
        n_layers = auto_layers if n_layers is None else n_layers
        offsets_km = auto_offsets if offsets_km is None else offsets_km
...
def edge_energy_wh(node_from, node_to, atmos_from, atmos_to, d: DroneSpecs,
                    elev_from: float = 0.0, elev_to: float = 0.0,
                    n_grid: int = 40, cache: dict = None):
    ...
    Also charges for climbing if elev_to > elev_from (see climb_power_w) --
    the lattice holds a constant height AGL, so it climbs/descends with the
    terrain underneath each edge.
    Returns (energy_wh, time_s, ground_speed_mps).

    `cache`, if given, is a dict shared across calls (e.g. across both the
    Dijkstra search and the straight-line comparison) so an edge that's
    evaluated more than once -- the two searches share every edge on the
    centerline -- is only ever solved once.
    """
    cache_key = (node_from, node_to) if cache is not None else None
    ...
    def energy_and_time(g):
        ...
        time_s = dist_m / g
        power_w = (
            propulsion_power_w(v_air, rho, d)
            + d.avionics_payload_power_w
            + climb_power_w(delta_h_m, time_s, d)
        )
    ...
    result = (energy_wh, time_s, best_g)
    if cache_key is not None:
        cache[cache_key] = result
    return result
...
def solve_min_energy_route(layers, atmos_lookup, elev_lookup, d: DroneSpecs, edge_cache: dict = None):
    ...
    while pq:
        ...
        for j, nxt in enumerate(layers[li + 1]):
            e_wh, t_s, _g = edge_energy_wh(
                node, nxt, atmos_lookup[node], atmos_lookup[nxt], d,
                elev_lookup.get(node, 0.0), elev_lookup.get(nxt, 0.0), cache=edge_cache,
            )
            new_cost = cost + e_wh
            key = (li + 1, j)
...
def straight_route_energy(layers, atmos_lookup, elev_lookup, d: DroneSpecs, edge_cache: dict = None):
    ...
    for i in range(len(layers) - 1):
        ...
        e, t, _g = edge_energy_wh(
            n1, n2, atmos_lookup[n1], atmos_lookup[n2], d,
            elev_lookup.get(n1, 0.0), elev_lookup.get(n2, 0.0), cache=edge_cache,
        )
        total_e += e
        total_t += t
...
def main():
    ...
    n_lanes = max(len(row) for row in layers)
    layer_spacing_km = dist_km / (len(layers) - 1) if len(layers) > 1 else dist_km
    ...
    print(f"Lattice: {len(layers)} layers x up to {n_lanes} lanes "
          f"(~{layer_spacing_km:.1f} km between layers, sized to this route's "
          f"distance -- see lattice_sizing())")
    ...
    # Wind/atmosphere and elevation are two independent HTTP calls to two
    # different Open-Meteo endpoints -- fire them concurrently instead of
    # paying both round trips back to back.
    with ThreadPoolExecutor(max_workers=2) as pool:
        wind_future = pool.submit(fetch_wind_and_atmos_batch, all_nodes)
        elev_future = pool.submit(fetch_elevations, all_nodes)
        atmos_lookup = wind_future.result()
        elevations = elev_future.result()
    elev_lookup = dict(zip(all_nodes, elevations))
    ...
    # by (node_from, node_to) means it's only ever solved once.
    edge_cache = {}
    straight_e, straight_t = straight_route_energy(layers, atmos_lookup, elev_lookup, DRONE, edge_cache)
    opt_e, opt_t, opt_path = solve_min_energy_route(layers, atmos_lookup, elev_lookup, DRONE, edge_cache)
