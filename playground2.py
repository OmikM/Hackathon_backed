...
class DroneSpecs:
    ...
    n_batteries: int = 2
    max_speed_mps: float = 23.0          # S-mode horizontal
    ...
    cruise_ground_speed_mps: float = 12.0  # ASSUMED: chosen cruise speed for this demo
...
def edge_energy_wh(node_from, node_to, atmos_from, atmos_to, d: DroneSpecs):
    ...
    ground_e, ground_n = bearing_to_vec(brg, d.cruise_ground_speed_mps)
    ...
    # ground_velocity = airspeed_velocity + wind_velocity  =>  airspeed = ground - wind
    air_e, air_n = ground_e - wind_e, ground_n - wind_n
    v_air = math.hypot(air_e, air_n)
    ...
    time_s = (dist_km * 1000) / d.cruise_ground_speed_mps
...
def solve_min_energy_route(layers, atmos_lookup, d: DroneSpecs):
    ...
    while pq:
        ...
        for j, nxt in enumerate(layers[li + 1]):
            e_wh, t_s = edge_energy_wh(node, nxt, atmos_lookup[node], atmos_lookup[nxt], d)
            new_cost = cost + e_wh
            key = (li + 1, j)
...
def straight_route_energy(layers, atmos_lookup, d: DroneSpecs):
    ...
    for i in range(len(layers) - 1):
        ...
        e, t = edge_energy_wh(n1, n2, atmos_lookup[n1], atmos_lookup[n2], d)
        total_e += e
        total_t += t
